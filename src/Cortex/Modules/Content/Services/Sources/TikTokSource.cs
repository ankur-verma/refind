...
public class TikTokSource : IContentSource
{
    ...
    private readonly ILogger<TikTokSource> _logger;
    ...
    public bool CanHandle(string url)
    {
        ...
        return uri.Host.ToLowerInvariant().Contains("tiktok.com");
    }
    ...
    public async Task<ContentExtractionResult> ExtractAsync(string url, CancellationToken ct = default)
    {
        ...
        if (_settings.UseGeminiVideoAnalysis)
        {
            ...
            if (mp4Result?.Mp4FilePath is not null)
            {
                ...
                result.Title = mp4Result.Title ?? geminiMetadata.Title ?? "TikTok content";
                result.Description = geminiMetadata.Description ?? "";
                result.Transcript = CleanText(geminiAnalysis);
                result.Tags = new List<string> { "social", "tiktok", "short-form" };
        ...
        if (extraction is not null)
        {
            ...
            result.Title = extraction.Title ?? "TikTok content";
        ...
        var metadata = await ReadHtmlMetadataAsync(_httpClient, url, "TikTok content", ct);
        
        result.Title = metadata.Title ?? "TikTok content";
